...
object Nino  {
  ...
  def validate(it: Nino): Boolean = {
    it.unwrap.trim match {
      case ninoValidatorRegEx(n) => validationCheck(n).isDefined
      case x                     => false
    }
  }

  def normalize(it: Nino): Nino = Nino(it.unwrap.toUpperCase.replaceAll(" ", ""))
}